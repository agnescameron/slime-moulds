...
void pickEdge(int array[52][102], int foodi, int foodj, int &edgei, int &edgej){
...
void foodPath(int array[52][102], int food, vector<vector<int> > foodLocator)
{
    ...
    //food cells randomly enter the plasmodium over a number of cycles
    //this results in net growth
    do{
            ...
            do{
                countSize(array);
                   ...
                   checkOnEdge(array, 2, foodi, foodj, isOnEdge);
                        if(isOnEdge == true){
                            pickEdge(array, foodi, foodj, edgei, edgej);
                            array[edgej][edgei] = 6;
                            print(array);
    ...
    //the phase in which the slime migrates towards food sources
     ...
     int simsteps = 0;  
       ...
       do{
         ...
         do{
          cout << "Pick a limit for the number of cycles"<< endl <<"(typically 10000)?: ";
		  cin >> limit;
  ...
  do{  
   do {
      ...
      do{
       ...
       migration (genNext, s, n, food, printBubble, foodLocator);
       cout << "printing" << endl;
       print (genNext);
       system("sleep .05");
       cout << endl;
